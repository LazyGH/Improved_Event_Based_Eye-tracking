...
#### comment parsers before running the model_test.ipynb
...
Frame = namedtuple('Frame', 'index row col img timestamp')
...
class EyeDataset:
    ...
    def __getitem__(self, index):
        ...
        # frame_timestamp = -np.Inf
        # event_timestamp = np.Inf
    ...
    def load_frame_data(self, eye):
        ...
        for fpath in img_filepaths:
            ...
            frame = Frame(path_info['index'], path_info['row'], path_info['col'], fpath, path_info['timestamp'])
...
def display_data(eye_dataset, opt):
    col_buffer = []
    row_buffer = []
    polarity_buffer = []
    s = plt.plot([], [])[0]
